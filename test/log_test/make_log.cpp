                     ...
                     int line, const std::string &funcname, LogLevel level, const char *fmt, ...)
{
  ...
  filepath = "./logs/" + std::string(module_name) + "/" + std::to_string(now_tm->tm_year + 1900) + "/" + std::to_string(now_tm->tm_mon + 1) + "/" + std::string(proc_name) + "-" + std::to_string(now_tm->tm_mday) + ".log";
