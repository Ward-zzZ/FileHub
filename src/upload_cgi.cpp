...
int recv_save_file(long len, char *user, char *filename, char *md5, long *p_size) {
    ...
    if (!file_buf) {
        LOG_ERROR(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "new file_buf error!");
        return -1;
    }
    ...
    */
    ...
    if (p == NULL) {
        LOG_ERROR(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "Wrong no boundary!\n");
        delete[]file_buf;
        return -1;
    }
    //拷贝分界线
    ...
    LOG_DEBUG(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "boundary: [%s]\n", boundary);
    p += 2;//跳过\r\n
    ...
    p = strstr(begin, "\r\n");//\r\n是行结束符
    if (p == NULL) {
        LOG_ERROR(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "ERROR: get context text error, no filename?\n");
        delete[]file_buf;
        return -1;
    }
    strncpy(content_text, begin, p - begin);
    boundary[p - begin] = '\0';   //字符串结束符
    LOG_DEBUG(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "content_text: [%s]\n", content_text);
    p += 2;//跳过\r\n
    len -= (p - begin);//已经处理了p-begin的长度
    ...
    *p_size = strtol(tmp, NULL, 10); //字符串转long
    ...
    if (p == NULL) {
        LOG_ERROR(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "memstr(begin, len, boundary) error\n");
        delete[] content_text;
        delete[] boundary;
        delete[]file_buf;
        return -1;
